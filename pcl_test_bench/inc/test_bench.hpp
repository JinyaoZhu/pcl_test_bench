...
public:
	...
	*/
	TestBench() {
		...
		std::string test_bench_name = "TestBench: " + reg_algorithm_name;
		visualizer_ptr->setWindowName(test_bench_name.c_str());
	...
	*/
	void computeResult() {
		...
		for (int i = 0; i < results.size(); ++i) {
			...
			Eigen::Matrix4f final_trans = results[i].final_transformation;
			Eigen::Matrix4f ref_translation = results[i].ref_transformation;
			...
			results[i].error_quat = error_q;
			results[i].error_trans = error_trans = error_transformation.block<3, 1>(0, 3);
	...
	*/
	void doAlignAll() {
		...
		vizResultAddOne<PointType>(nullptr,0);
